...
class TOCExtractor:
    ...
    def extract_toc_from_epub(self, epub_path: str) -> List[TOCEntry]:
        ...
        with zipfile.ZipFile(epub_path, "r") as epub:
            ...
            html_files = [
                f for f in epub.namelist() if f.endswith((".html", ".xhtml", ".htm"))
            ]
    ...
    def _is_potential_heading(self, text: str, span: dict, font_stats: dict) -> bool:
        ...
        # Pattern matching
        pattern_match = any(
            re.match(pattern, text, re.IGNORECASE) for pattern in self.heading_patterns
        )

        # Keyword matching
        keyword_match = any(
            keyword in text.lower() for keyword in self.heading_keywords
        )
