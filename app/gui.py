...
class IdlearnApp(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.summarize_checkbox = ctk.CTkCheckBox(self, text="Summarize", command=None, hover=True,
                                                variable=self.summarize, onvalue="on", offvalue="off")
        self.summarize_checkbox.grid(row=2, column=0, padx=20, pady=(0,0))
    ...
    def make_outfolder(self):
        ...
        if self.pipeline:
            self.pipeline.output_folder = self.outfolder


    def make_run(self):
        if self.filename is not None:
            if self.outfolder is not None:
                # Disable run button to prevent multiple simultaneous executions
